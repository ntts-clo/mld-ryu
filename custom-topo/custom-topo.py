...
class CustomTopo(Topo):
    ...
    def set_ofp_version(self, switch, protocols):
        ...
        command = 'ovs-vsctl set Bridge %s protocols=%s' \
                  % (switch, protocols_str)
    ...
    def exec_usercmd(self, cmd_file):
        ...
        for line in fh:
            ...
            (host_name, cmd) = dmy
            host = None
            if host_name in self.switchList:
                host = self.switchList[host_name][2]
            elif host_name in self.hostList:
...
if '__main__' == __name__:
    ...
    if False == topo.start_networking():
        print >> sys.stderr, \
            "ERROR: Failed to set network parameter for %s." % defFile
        exit
