...
from ryu.lib.packet import packet, icmpv6, vlan, lldp
...
# Ryu MLDコントローラー
# =============================================================================
class lldp_controller(app_manager.RyuApp):
    # OpenFlowのバージョン用定数
    ...
    def __init__(self, *args, **kwargs):
        try:
            ...
            super(lldp_controller, self).__init__(*args, **kwargs)
...
#            self.send_to_mld(dispatch_)
    ...
    def _packet_in_handler(self, ev):
        ...
        try:
            ...
            self.logger.debug("in_port:%s", str(msg.match['in_port']))

#            # CHECK LLDP
#            pkt_lldp = pkt.get_protocol(lldp.lldp)
#            if not pkt_lldp:
#                self.logger.debug("check lldp:None \n")
#                return False
#            self.logger.debug("pkt_lldp : %s", pkt_lldp)
...
#            self.send_to_mld(dispatch_)
    ...
    # send_lldp_reg
    # =========================================================================
    def send_lldp_reg(self):
        self.logger.debug("")
        hub.sleep(2)

        port_no = 1
#        port_no = 2
        dl_addr = "d4:3d:7e:4a:43:fd"

        while True:
            if len(self.datapath) == 3:

                datapath = self.datapath.get(1)
#                datapath = self.datapath.get(276595101184)
                if datapath is None:
                ...
                lldp_data = LLDPPacket.lldp_packet(
                    datapath.id, port_no, dl_addr, self.DEFAULT_TTL)
                output_port = parser.OFPActionOutput(port_no, ofproto.OFPCML_NO_BUFFER)
                packet_out = parser.OFPPacketOut(
                    datapath, ofproto.OFPP_ANY, ofproto.OFPP_CONTROLLER,
                    [output_port], lldp_data)
                datapath.send_msg(packet_out)
                self.logger.debug("sended packet : %s", packet_out)
        #
        #        datapath_id = 0
        #        port_no = 0
        #        dl_addr = "56:28:b2:95:a4:4d"
        #        lldp_data = LLDPPacket.lldp_packet(
        #            datapath_id, port_no, dl_addr, self.DEFAULT_TTL)
        #
        #        # send of scapy
        #        sendrecv.sendp(
        #            scapy_packet.Packet(lldp_data), iface="eth0", verbose=0)
            else:
                self.logger.debug("datapath is not 3(%d).", len(self.datapath))
